...
function initIridescence() {
    const ctnDom = document.getElementById('iridescence-bg');
    if (!ctnDom) return;
    ...
    function resize() {
        const scale = 1;
        renderer.setSize(ctnDom.offsetWidth * scale, ctnDom.offsetHeight * scale);
        if (program) {
    ...
    ctnDom.addEventListener('mousemove', handleMouseMove);
